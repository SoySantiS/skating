...
public class PointsManager : MonoBehaviour
{
    ...
    void FinalCountOfPoints()
    {
        ...
        {
            if (checkPoint.Seconds == 0)
            {
                AddPoints(checkPoint.Points);
                PlayerPrefs.SetInt("Last Score", amount);
                extraPoints = true;
            }else if (Timer.SharedInstance.Seconds <= checkPoint.Seconds)
            {
                AddPoints(checkPoint.Points);
                PlayerPrefs.SetInt("Last Score", amount);
                extraPoints = true;
            }else if (Timer.SharedInstance.Seconds > checkPoint.Seconds && Timer.SharedInstance.Minutes < checkPoint.Minutes)
            {
                AddPoints(checkPoint.Points);
                PlayerPrefs.SetInt("Last Score", amount);
                extraPoints = true;
